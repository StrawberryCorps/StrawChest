package bzh.strawberry.chest.api.utils;
    ...
    public static String toJson(ItemStack itemStack) {
        ...
        if (itemStack.hasItemMeta()) {
            ...
            if (!meta.getPersistentDataContainer().isEmpty()) {
                JsonArray namespace = new JsonArray();

                if (meta.getPersistentDataContainer().has(new NamespacedKey(StrawChest.getInstance(), "coffre"), PersistentDataType.STRING)) {
                    namespace.add(meta.getPersistentDataContainer().get(new NamespacedKey(StrawChest.getInstance(), "coffre"), PersistentDataType.STRING));
                    metaJson.add("container", namespace);
                }
        ...
        if (element.isJsonObject()) {
            ...
            if (typeElement.isJsonPrimitive()) {
                ...
                if (itemMetaElement != null && itemMetaElement.isJsonObject()) {
                    ...
                    JsonElement container = metaJson.get("container");
                    ...
                    if (container != null && container.isJsonArray()) {
                        JsonArray jarray = container.getAsJsonArray();
                        jarray.forEach(jsonElement -> {
                            if (jsonElement.isJsonPrimitive()) {
                                String nom = jsonElement.getAsString();
                                meta.getPersistentDataContainer().set(new NamespacedKey(StrawChest.getInstance(), "coffre"), PersistentDataType.STRING, nom);
                            }
                        });
                    }
